...
function OrderDetailPage() {
  ...
  const [id, setId] = useState(result.get('id'))
  ...
  const recordsColumns: ColumnsType<DataType> = [
    ...
    },
    {
      title: '支付渠道',
      width: 300,
      render: (_, record: any) => <span>{record.payTypeText}</span>,
    },
    {
      title: '支付金额',
      render: (_, record: any) => (
  ...
  const goodsColumns: ColumnsType<DataType> = [
    ...
    // },
    {
      title: '商品ID',
      width: 120,
      render: (_, record: any) => <span>{record.goodsId}</span>,
    },
    {
      title: '商品',
