...
import { Button, Col, Dropdown, Form, Input, Row, message } from 'antd'
...
function CertificateCreatePage() {
  ...
  const getImgInfo = () => {
    ...
    img.onload = () => {
      ...
      const valueX = 0.5 * (window.screen.width - size * img.width)
      const valueY = 106
      setDragX(valueX)
      setDragY(valueY)
    }
  ...
  const onFinish = (values: any) => {
    ...
    let relation: any = []
    if (coursesData.length > 0)
      relation = relation.concat(coursesData)
  ...
  const changeSize = (val: number) => {
    ...
    if (val === -1) {
      ...
      const newSize = sizeRef.current - 0.25
      setSize(newSize)
              ...
              >
                ...
                >
                  <UploadImageButton
                    text={uploadName}
                    onSelected={(url) => {
                      form.setFieldsValue({ templateImage: url })
                      setThumb(url)
                    }}
                  >
                  </UploadImageButton>
