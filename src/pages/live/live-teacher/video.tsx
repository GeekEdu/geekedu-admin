...
function LiveVideoPage() {
  ...
  useEffect(() => {
    if (config) {
      setEnabledBulletSecret(
        Number.parseInt(config.player.enabledBulletSecret) === 1,
      )
      ...
      setBulletSecretFontSize(`${config.player.bulletSecret.size}px`)
      setBulletSecretOpacity(config.player.bulletSecret.opacity)
    }
  }, [config])
  ...
  const [title, setTitle] = useState(result.get('title'))
  ...
  const getData = () => {
    live.play(id).then((res: any) => {
      ...
      // 签到相关
      const sign_in_record = resData.sign_in_record
  ...
  const renderLiveTencentPlayer = () => {
    ...
    liveTencentPlayerRef.current.on('error', (e: any) => {
  ...
  const renderLiveDPlayer = () => {
    ...
    liveDPlayerRef.current.on('timeupdate', () => {
      ...
      livePlayRecord(Number.parseInt(liveDPlayerRef.current.video.currentTime), false)
    })
  ...
  const goDetail = () => {
    ...
    setClose(true)
    setTimeout(() => {
      navigate(`/live/course/video/index?id=${courseId}&title=${title}`, { replace: true })
    }, 500)
  }
  ...
  const initVodPlayer = () => {
    ...
    vodDlayerRef.current.on('timeupdate', () => {
      playRecord(Number.parseInt(vodDlayerRef.current.video.currentTime), false)
    })
    vodDlayerRef.current.on('ended', () => {
      playRecord(Number.parseInt(vodDlayerRef.current.video.currentTime), true)
    })
          ...
          {isLogin && video && (
            <div className={styles['live-box']}>
              <div className={styles.obs}>
                <div className={styles.obsServer}>
                  <Input 
                    className={styles.server}
                    value='https://webrtcpush.tlivesource.com/webrtc/v2/whip'
                  />
                  <div className={styles['server-copy']}>Copy</div>
                </div>
                <div className={styles.obsToken}>
                  <Input 
                    className={styles.token}
                    value={liveUrl}
                  />
                  <div className={styles['token-copy']}>Copy</div>
                </div>
              </div>
