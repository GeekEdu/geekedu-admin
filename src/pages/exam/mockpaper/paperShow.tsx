...
function MockPaperReadPage() {
  ...
  const getData = () => {
    mock.mockPaperJoinRecord(id, pid).then((res: any) => {
      ...
      const normaldata = res.data.questions
  ...
  const download = () => {
    const shareContent: any = document.querySelector('#pdfDom')
    const imgList = shareContent.querySelectorAll('img')
    if (imgList) {
      let i
